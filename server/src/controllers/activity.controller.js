...
const dsaTotal = dsaSheet.modules.reduce(
  (sum, mod) =>
    sum + mod.topics.reduce((topicSum, t) => topicSum + t.problems.length, 0),
  0,
);
...
export async function dashboard(req, res) {
  const [user, today, dsaSolved, systemSolved] = await Promise.all([
    ...
    SystemDesignProgress.countDocuments({
      userId: req.userId,
      completed: true,
    }),
  ]);
  const overall = Math.round(
    ((dsaSolved + systemSolved) / (dsaTotal + systemDesignTopics.length)) * 100,
  );
